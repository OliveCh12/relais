import { MediaStream, RTCPeerConnection, RTCRtpSender } from 'react-native-webrtc';
import { createSession, pollDescription, signalRequest } from '../signaling/client';
import { parseDescription, privateLanOrigin, type PairingDescriptor } from '../signaling/protocol';
import { parseMessage } from '../capture/protocol';
...
export class PeerSession {
  ...
  constructor(
    ...
    this.peer.addEventListener('connectionstatechange', () => {
      ...
      if (['failed', 'disconnected'].includes(this.peer.connectionState)) {
        this.dispose('Connection lost. Keep Camera open to reconnect.');
      }
    });
  ...
  private attachChannel(channel: RTCDataChannel) {
    ...
    const update = () => {
      ...
      if (channel.readyState === 'closed') {
        this.dispose('The camera connection closed. Keep both apps open to reconnect.');
        return;
      }
    ...
    channel.addEventListener('message', ({ data }) => {
      ...
      if (this.handshake?.isPaired && typeof data === 'string') this.callbacks.message?.(data);
      const message = parseMessage(data);
      if (!message || channel.readyState !== 'open') return;
      if (message.type === 'ping') channel.send(JSON.stringify({ type: 'pong', id: message.id }));
      if (message.type === 'pong' && this.pendingPing && message.id === this.pendingPing.id) {
  ...
  private prepareHandshake(role: 'camera' | 'monitor', server: string, expected?: SavedDevice) {
    this.handshake = new DeviceHandshake({
      ...
      remembered: (device) => {
        this.callbacks.trusted?.(true);
        this.callbacks.remembered?.(device);
      },
      error: (message) => this.dispose(message),
    });
  }
  ...
  async startCamera(serverInput: string, openCamera: () => Promise<MediaStream>) {
    ...
    this.attachChannel(this.peer.createDataChannel('relais-camera', { ordered: true }));
  ...
  send(text: string) {
    if (!this.handshake?.isPaired || this.channel?.readyState !== 'open')
