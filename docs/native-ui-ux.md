...
## App settings and camera settings navigation — September 11, 2026

Home contains the two grouped role rows and a Settings gear in the native app bar, with no app-name heading. Settings contains Preferred quality, Connection settings and About. App settings use a normal native stack, not a modal. The iOS presentation uses SwiftUI Form/Section/Button and SF Symbols; Android uses Compose Card/ListItem/RadioButton/Switch and individually imported Material Symbols. Shared section data describes intent while platform components render it. [Expo UI 57](https://docs.expo.dev/versions/latest/sdk/ui/), [Material cards](https://developer.android.com/develop/ui/compose/components/card).

A saved camera has one header availability indicator and Connect in the app bar. Connection, Capture, Video, Brightness, Photo, Preset and Monitor preview open stack pages as applicable. Enumerated remote settings open selection pages with native checkmarks or radio controls. Unsupported controls are omitted, and a changed mode or lens may require a connection to refresh the format catalogue. There is no separate Info page. Measured link diagnostics run only while the Connection child page is focused.
...
### Camera Settings submenu and list icons

The device overview contains Connection, a single Camera Settings entry, Monitor preview and Forget. Camera Settings opens a stack page containing the available Capture, Video, Photo and Brightness categories plus Preset; each leads to another stack page. The overview does not enumerate camera controls, and this hierarchy uses no accordion or dropdown.

Navigation rows share a SettingsIcon presentation: SF Symbols in compact colored rounded squares on iOS, Material Symbols in tonal circles on Android. Category colors are centralized, with a neutral treatment for offline devices. Home, My cameras, app Settings, device settings and the pairing navigation reuse these components. Visible labels and availability text remain the source of meaning; color is supplementary. Native list containers, selection controls and system Back navigation remain in place.
