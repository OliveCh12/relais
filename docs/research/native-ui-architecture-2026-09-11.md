...
This section records the initial code and dependency review informed by the user's screenshots. See [the subsequent simulator audit](../qa/simulator-audit-2026-09-11.md) for observed behavior and build corrections. The current Pixel capture was black; its window state confirmed a locked screen. It is not visual evidence of Relais. No current two-phone visual, screen-reader or frame-rate audit is claimed.

1. **Android typography lost native context.** Expo UI 57.0.17's `TextView.kt` used `TextStyle.Default` when no explicit style was provided, overriding the typography supplied by Material ListItem/Button slots. The versioned pnpm patch now inherits `LocalTextStyle.current`. Simulator QA subsequently established that Android was still linking Expo UI’s prebuilt AAR, which bypassed this source patch. `expo.autolinking.android.buildFromSource: ["expo-ui"]` now opts this Gradle project out of prebuilt binaries; the build must execute `:expo-ui:compileReleaseKotlin` before claiming the patch is included. This repairs text size, weight and line-height inheritance at the binding, rather than adding a font size to every button.
